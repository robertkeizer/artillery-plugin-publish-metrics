...
function DatadogReporter(config, events, script) {
  ...
  events.on('stats', (stats) => {
    ...
    if( report.scenarioDuration && Object.keys(report.scenarioDuration).length > 0 ){
      for( var durationName in report.scenarioDuration ){
        const _key = "duration." + durationName;
        const _val =  report.scenarioDuration[durationName];
        metrics.gauge(_key, _val );
      }
    }
